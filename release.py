...
def repo_compare(old: Optional[Version] = None, new: Optional[Version] = None) -> str:
    return (
        f"[{new or 'Unreleased'}]: http://github.com/jacksmith15/"
        f"radish/compare/{old or 'initial'}..{new or 'HEAD'}\n"
    )
...
def get_unreleased(current_version: Version) -> Tuple[Version, str]:
    with open(CHANGELOG, "r", encoding="utf8") as file:
        ...
        unreleased_lines = [*takewhile(consume_to_version(current_version), file)]
...
def checkout_master():
    try:
        ...
        raise RuntimeError('You have uncommitted changes, "git stash" to save them')
...
def verify_tag():
    color_lines = "\n".join([color_line(line) for line in bash("git diff").split("\n")])
    return bool_input(
        f"""
...
def verify_release(current_version: Version, next_version: Version, content: str) -> bool:
    return bool_input(
        f"""
...
def github_release_body(current_version: Version, next_version: Version):
    return f"""
[Release Notes](https://github.com/jacksmith15/radish/blob/{next_version}/CHANGELOG.md)
[Changes](http://github.com/jacksmith15/radish/compare/{current_version}..{next_version})
"""


def main():
    header("Starting release")
    if not bool_input(f"This will checkout master and perform release, continue?"):
        sys.exit(1)
    checkout_master()
    ...
    print("Github 'release' body:" f"{github_release_body(current_version, next_version)}")
