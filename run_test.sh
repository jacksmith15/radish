...
lint() {
    header "Linting" "$blue"
    pyflakes radish || EXIT_CODE=1
    pyflakes tests || EXIT_CODE=1
}

typecheck() {
    header "Checking types" "$blue"
    mypy --ignore-missing-imports radish tests || EXIT_CODE=1
}

tests() {
    header "Running unit tests" "$blue"
    pytest -v -s --junitxml=unit_test_results.xml --cov="radish" --cov-append --cov-branch --cov-report= tests || EXIT_CODE=1
}
