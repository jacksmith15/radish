...
class FilterFactory(Generic[Model]):
    """A factory for generating resource specific filters.

    Used by :func:`~radish.resource.Resource` - should not be instantiated directly.
    """
...
class Filter(Generic[FieldType]):
    """Simple function wrapper to aid filter composition.

    Filters can be composed using the ``&`` and ``|`` operators.
    """
...
def within(values: Iterable[Any]) -> Filter:
    """Match when a field value is a member of a container."""
    return Filter(lambda value: value in values, repr=f"{within.__name__}({repr(values)})")


def contains(value: Any) -> Filter:
    """Match when an iterable field value contains a given value."""
    return Filter(
